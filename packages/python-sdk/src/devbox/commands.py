...
class CommandHandle:
    """A running process that can receive input, signals, or be reconnected."""

    def __init__(
        self,
        pid: int,
        commands: Commands,
        events: Generator[Mapping[str, Any], None, None] | None = None,
        *,
    ...
    def wait(
        ...
        *,
        ...
        if self._result is None:
            ...
            self._events = None
            try:
                self._result = self._commands._collect_events(
    ...
    def send_stdin(self, data: str | bytes) -> None:
        """Send data to the process standard input."""
        self._ensure_active()
        self._commands._send_input(self.pid, data, self._input_stream)

    def close_stdin(self) -> None:
        """Close the process standard input stream."""
        self._ensure_active()
    ...
    def disconnect(self) -> None:
        """Close the local stream without stopping the remote process."""
    ...
    def _ensure_active(self) -> None:
        if self._result is not None:
            raise RuntimeError("process has already completed")
...
class Commands:
    ...
    def connect(self, pid: int, *, timeout: float | None = 60) -> CommandHandle:
        """Reconnect to a running process by process ID."""
        _validate_pid(pid)
    ...
    def list(self) -> tuple[ProcessInfo, ...]:
        """List processes managed by EnvD in this sandbox."""
    ...
    def send_stdin(self, pid: int, data: str | bytes) -> None:
        """Send standard input to a process by process ID."""
        self._send_input(pid, data, "stdin")

    def close_stdin(self, pid: int) -> None:
        """Close standard input for a process by process ID."""
        _validate_pid(pid)
...
class AsyncCommandHandle:
    """Asynchronous counterpart of :class:`CommandHandle`."""

    def __init__(
        self,
        pid: int,
        commands: AsyncCommands,
        events: AsyncGenerator[Mapping[str, Any], None] | None = None,
        *,
        input_stream: str = "stdin",
        reconnect_timeout: float | None = 60,
    ) -> None:
...
class AsyncCommands:
    ...
    async def _collect_events(
        events: AsyncGenerator[Mapping[str, Any], None],
        pid: int,
        ...
        async for event in events:
            for chunk in collector.add(event):
                if chunk.stream == "stdout" and on_stdout:
                    await _invoke(on_stdout, chunk.data)
                if chunk.stream == "stderr" and on_stderr:
...
class _OutputCollector:
    ...
    def add(self, response: Mapping[str, Any]) -> tuple[OutputChunk, ...]:
            ...
            for stream in ("stdout", "stderr", "pty"):
                if stream in data:
                    ...
                    if value:
                        ...
                        chunks.append(OutputChunk(stream=target, data=value))
        end = event.get("end")
...
async def _first_pid_async(events: AsyncGenerator[Mapping[str, Any], None], action: str) -> int:
    try:
